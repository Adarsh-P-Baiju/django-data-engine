...
from import_engine.api.file_validators import validate_file_size, validate_file_extension
...
def handle_upload(model_name: str, uploaded_file) -> ImportJob:
    ...
    try:
        ...
        if not is_clean:
            raise ValidationError(f"Security Alert: File infected with {virus_name}. Upload rejected.")
