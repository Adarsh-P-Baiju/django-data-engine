    ...
    def prefetch(self, rows_data: List[Dict[str, Any]]):
        ...
        for f_name, f_config in self.fk_fields.items():
            ...
            if not fk_reg_config:
                logger.warning(
                    f"FKResolver: No config found for registered FK '{fk_name}'"
                )
                continue
            ...
            if f_config.get("create_if_missing"):
                ...
                if missing_values:
                    self._handle_missing_creation(
                        f_name, model, lookup_field, missing_values, f_config
                    )

    def _handle_missing_creation(
        self, f_name, model, lookup_field, missing_values, f_config
    ):
