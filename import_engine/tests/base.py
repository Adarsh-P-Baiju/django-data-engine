...
class BaseImportTestCase(TransactionTestCase):
    ...
    def assertJobStatus(self, job_id, expected_status):
        ...
        self.assertEqual(job.status, expected_status, f"Job {job_id} expected status {expected_status}, got {job.status}")

    def assertLogCount(self, job_id, expected_count):
        count = ImportLog.objects.filter(job_id=job_id).count()
        self.assertEqual(count, expected_count, f"Job {job_id} expected {expected_count} logs, got {count}")
