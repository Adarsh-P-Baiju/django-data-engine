...
class ResumableUploadView(APIView):
    ...
    def post(self, request, model_name):
        ...
        if not filename or not total_size:
            return Response({"error": "filename and total_size required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"error": "Failed to initialize upload session"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def patch(self, request, job_id):
        ...
        if not content_range:
            return Response({"error": "Content-Range header required (e.g., bytes 0-1023/2048)"}, 
        ...
        try:
            ...
            return Response({"error": "Invalid Content-Range format"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if new_offset >= job.total_bytes:
                ...
                job.save(update_fields=["processed_bytes", "status", "status_message", "file_fingerprint"])
                return Response({"status": "COMPLETED", "message": "File fully received. Processing..."}, status=status.HTTP_200_OK)
            else:
                job.save(update_fields=["processed_bytes"])
                return Response({
                    "status": "PARTIAL",
                    "next_offset": new_offset,
                    "percent_complete": round((new_offset / job.total_bytes) * 100, 2)
                }, status=status.HTTP_206_PARTIAL_CONTENT)

        except Exception as e:
            logger.error(f"Chunk Patch Failed for {job_id}: {e}")
            return Response({"error": "Chunk Write Failure"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
