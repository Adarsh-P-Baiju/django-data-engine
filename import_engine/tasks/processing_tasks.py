    ...
    try:
        ...
        for row_idx, row_dict in rows_data:
            if job.field_mapping:
                ...
                label_to_field = {
                    (
                        f_config.get("label") if isinstance(f_config, dict) else f_name
                    ): f_name
        ...
        for row_idx, row_dict in rows_data:
            ...
            if errors:
                ...
                if "department" in cleaned_data:
                    logger.error(
                        f"STRANGE: department still in cleaned_data! keys={list(cleaned_data.keys())}, fk_fields={list(resolver.fk_fields.keys())}"
                    )
        ...
        with transaction.atomic():
            if instances_to_create:
                ...
                if conflict_res == "update":
                    ...
                    bulk_persist(
                        model_class, instances_to_create, upsert_fields=upsert_fields
                    )
                elif conflict_res == "ignore":
                    bulk_persist(
                        model_class, instances_to_create, ignore_conflicts=True
                    )
                else:
        ...
        if (
            ...
            try:
                ...
                async_to_sync(channel_layer.group_send)(
                    f"job_{job.id}", {"type": "job_progress", "payload": payload}
                )
        ...
        try:
            ...
            channel_layer = get_channel_layer()
            async_to_sync(channel_layer.group_send)(
                f"job_{job.id}", {"type": "job_progress", "payload": err_payload}
            )
        except Exception:
            pass
...
def check_job_completion(job_id):
    ...
    if all(c.status == ImportChunk.Status.DONE for c in chunks):
        ...
        if job.failure_count > 0 and job.success_count == 0:
            job.status = ImportJob.Status.FAILED
            job.error_message = (
                f"All {job.failure_count} rows failed import validation."
            )
        else:
